...
# Required
import re , random, string, os, requests, json
from typing import Dict, Any
from colorama import Fore, Back, Style
clear = lambda: os.system('cls')
# Load config file
config_file = 'furina_config.json' 
with open(config_file, 'r') as file:
    config_data = json.load(file)
driver_path = config_data['driver_path']
XAGtoken = config_data['XAGtoken']
CachedXBL = config_data['xbltoken']
minTypeSpeed = float(config_data['minTypeSpeed'])
maxTypeSpeed = float(config_data['maxTypeSpeed'])
defaultTimeout = int(config_data['defaultTimeoutinSeconds'])
if XAGtoken == "":
    print(Fore.RED + "XAGtoken is not set in config file. This is required, (yes even if you're not using XAG LOL, just put a random string.).")
...
# Modify chrome options if you want. (honestly, dont touch if you dont know what you're doing)
...
print(Fore.GREEN + " ")
print(Fore.LIGHTCYAN_EX + "[XGP Tool]")
print(Fore.GREEN + " ")
print(Fore.LIGHTCYAN_EX + "[1] Pre-check your codes (Threadless)")
print(Fore.LIGHTCYAN_EX + "[2] Redeemer")
print(Fore.LIGHTCYAN_EX + "[3] Set profile")
print(Fore.LIGHTCYAN_EX + "[9] View config")
print(Fore.LIGHTRED_EX + "+ Changelog: Lolz")
print(Style.RESET_ALL) 
...
def Type_Me(element: WebElement, text: str):
    for character in text:
        ...
        sleep(uniform(minTypeSpeed, maxTypeSpeed)) # If you want you can change .02 and .06 to lower/higher values to increase/decrease typing speed on Sign In page
wait = WebDriverWait(driver, defaultTimeout) # Default timeout.
def generateAccount():
    ...
    while True:
        try:
            ...
            if response.status_code == 429:
                print(Fore.RED + "Rate limited, exiting")
                exit()
...
def mc_token(session, uhs, xsts_token):
    ...
    while True:
        try:
            ...
            continue
def setProfile(email,password,name):
    print(Fore.YELLOW + 'Attempting to set profile: ' + email + ':' + password + " | " + name)              
    ...
    if "ACTIVE" in str(response_json):
                        print(Fore.GREEN + "+ " + email + ' + Name changed to ' + name)
                        print(Style.RESET_ALL)
                        exit()
    else:
                        print(Fore.RED + "- " + email + ' - Name change failed: ')
                        print(response_json)
                        print(Style.RESET_ALL)
                        exit()
def authenticate(email, password, tries = 0):
    ...
    try:
       ...
       print(Fore.RED+f"Failed to auth, check if you're phonelocked or not. : {email}:{password}")
    finally:
        session.close()
def fetchAccount():
    global emailid
...
Input = input("> ")
if Input == "1":
    choice = input(Fore.BLUE + "[1] = Load the cached token \n[2] = Use new token, save it to cache \n > ")
    if choice == "2":
        xbltoken = input(Fore.BLUE + "Enter your XBL token:")
        with open('furina_config.json', 'r') as file:
            config = json.load(file)
        file.close()
        config['xbltoken'] = xbltoken
        with open('furina_config.json', 'w') as file:
            json.dump(config, file, indent=4)
        preCheckCodes(xbltoken)
    if choice == "1":
        preCheckCodes(CachedXBL)
if Input == "2":
    redeemer()
    driver.quit()
...
if Input == "3":
    ...
    paswd = input(Fore.GREEN + "Enter password: " + Style.RESET_ALL)
    usern = input(Fore.GREEN + "Enter username: " + Style.RESET_ALL)
    setProfile(email, paswd, usern)
if Input == "9":
    with open('furina_config.json', 'r') as file:
        config = json.load(file)
    config['XAGtoken'] = '*********'
